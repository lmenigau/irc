...
void    privmsg( std::list<std::string>* args, Client &c)
{
    ...
    for (std::vector<Client *>::iterator it = ircserv::_clients.begin(); it != ircserv::_clients.end(); it++)
    {
        if ((*it)->getNick() == args->front())
        {
            ...
            //std::cout << send((*it)->getFd(), buf, args->back().length(), 0) << std::endl;
            (*it)->reply( format (":%s!~%s PRIVMSG %s: %s\r\n", c.getNick().c_str(), (c.getUser() + "@" + c.getHostname()).c_str(), target.c_str(), args->back().c_str()));
        }
    }
