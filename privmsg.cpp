...
void privmsg_client( std::list<std::string>* args,
                     Client&                 c,
...
void privmsg_channel( std::list<std::string>* args,
                      Client&                 c,
                      const std::string&      target ) {
	...
	if ( it == channels.end() ) {
		c.reply( format( "%s!~%s@%s 403 %s :No such channel\r\n",
		                 ...
		                 c.getHostname().c_str(), target.c_str() ) );
		return;
	} else {
		for ( t_map_string_client_ref::iterator cli_list =
		          it->second.getClients().begin();
		      cli_list != it->second.getClients().end(); cli_list++ ) {
			std::string rep =
			    format( ":%s!~%s@%s PRIVMSG %s: %s\r\n", c.getNick().c_str(),
			            c.getUser().c_str(), c.getHostname().c_str(),
			            target.c_str(), args->back().c_str() );
			// std::cout << cli_list->second->getNick() << ": " << rep <<
			// std::endl;
			if ( cli_list->second != &c )
				cli_list->second->reply( format(
				    ":%s!~%s@%s PRIVMSG %s : %s\r\n", c.getNick().c_str(),
				    c.getUser().c_str(), c.getHostname().c_str(),
				    target.c_str(), args->back().c_str() ) );
		}
