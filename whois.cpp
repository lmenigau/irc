...
//Who is channel, status of the nick in the channel / all channel and status display
//Who is host, displaying the host name / ip 378 
//Who is mode, displaying the modes 379
//RPL_AWAY  301, is away
...
void    whois( std::list <std::string> *args, Client &c)
{
    ...
    if (if_nick_set(args->front()))
    {
        ...
        c.reply( format(":irvserv.localhost 311 %s %s %s* : %s\r\n", c.getNick().c_str(),
                        c.getUser().c_str(), c.getHostname().c_str(), c.getRealUser().c_str()));
